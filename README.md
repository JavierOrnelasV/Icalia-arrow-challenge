...
* **Production** - [https://Icalia-arrow-challenge-production.herokuapp.com](https://Icalia-arrow-challenge-production.herokuapp.com)
* **Staging** - [https://Icalia-arrow-challenge-staging.herokuapp.com](https://Icalia-arrow-challenge-staging.herokuapp.com)
...
* [Github repo](https://github.com/IcaliaLabs/Icalia-arrow-challenge)
...
```bash
$ git clone git@github.com:IcaliaLabs/icalia_arrow_challenge.git

```
2. Copy the `example.env` file to `.env` in the project's source directory.

```bash
$ cd icalia-arrow-challenge
$ cp example.env .env
```
...
That command will lift every service Icalia-arrow-challenge needs, such as the `rails server`, `postgres`, and `redis`.
...
In order to stop Icalia-arrow-challenge as a whole you can run:
...
You probably won't be working with a blank database, so once you are able to run Icalia-arrow-challenge you can restore the database, to do it, first stop all services:
...
```
% docker exec -ti icalia_arrow_challenge_db_1 bash
```

This will open up a bash session in to the database container.

Up to this point we just need to download a database dump and copy under `Icalia-arrow-challenge/backups/`, this directory is mounted on the container, so you will be able to restore it with:

```
root@a3f695b39869:/# bin/restoredb icalia_arrow_challenge_dev db/backups/<databaseDump>
```
